        ...
        <div>
          ...
          <div className="mt-5 flex gap-3">
            {[Twitter, Linkedin, Github].map((Icon, i) => (
              <a key={i} href="#" className="h-9 w-9 rounded-full border border-white/15 flex items-center justify-center text-white/60 hover:bg-coral hover:text-white hover:border-coral transition">
...
const FooterCol = ({ title, links }: { title: string; links: string[] }) => (
  <div>
    <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-coral mb-4">{title}</div>
