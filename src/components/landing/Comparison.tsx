...
const cell = (v: boolean | string) => {
  if (v === true) return <Check className="h-4 w-4 text-coral mx-auto" strokeWidth={3} />;
  if (v === false) return <X className="h-4 w-4 text-charcoal-muted/40 mx-auto" />;
    ...
    <section className="py-24 bg-cream-warm">
      <div className="container max-w-[900px]">
        <div className="text-center mb-10">
          <div className="inline-block text-[11px] font-bold text-coral uppercase tracking-[0.2em]">Comparison</div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-charcoal/10 bg-cream/50">
                <th className="text-left p-4 font-bold text-charcoal-muted text-[11px] uppercase tracking-wider">Feature</th>
                <th className="p-4 font-bold text-charcoal-muted text-[11px] uppercase tracking-wider">Traditional ATS</th>
                <th className="p-4 font-bold text-charcoal-muted text-[11px] uppercase tracking-wider">HireVue / Jobma</th>
                <th className="p-4 font-display font-bold text-coral text-[12px] uppercase tracking-wider border-l-2 border-coral bg-coral/5">HireIQ</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} className="border-b border-charcoal/5 last:border-0">
                  ...
                  <td className="p-4 text-center border-l-2 border-coral bg-coral/5">{cell(r[3])}</td>
                </tr>
              ))}
            </tbody>
          </table>
