          ...
          {plans.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`text-[11px] font-bold uppercase tracking-[0.2em] ${isHighlight ? "text-coral" : "text-charcoal-muted"}`}
                >
                  {p.name}
                </div>
                <div className="mt-4 flex items-end gap-1.5">
                  <span
                    className={`font-display font-bold text-5xl ${isHighlight ? "text-white" : "text-charcoal"}`}
                  >
                    {p.price}
                  </span>
                  <span
                    className={`pb-2 text-sm ${isHighlight ? "text-white/60" : "text-charcoal-muted"}`}
                  >
                    {p.sub}
                  </span>
                </div>
                ...
                <ul className="mt-7 space-y-3">
                  {p.features.map((f) => (
                    <li
                      key={f}
                      className={`flex items-start gap-2.5 text-sm ${isHighlight ? "text-white/85" : "text-charcoal"}`}
                    >
                      <Check
                        className={`h-4 w-4 flex-none mt-0.5 ${isHighlight ? "text-coral" : "text-coral"}`}
                        strokeWidth={3}
                      />
                      {f}
                    </li>
