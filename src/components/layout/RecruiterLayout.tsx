...
import { LayoutGrid, Briefcase, ClipboardList, Settings, Plus, ChevronDown, LogOut, PanelLeftClose, PanelLeftOpen } from "lucide-react";
...
const SidebarItem = ({ Icon, label, path, badge, collapsed = false }: { Icon: any; label: string; path: string; badge?: string; collapsed?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      {badge && (
        <span className={`rounded-full bg-destructive text-[10px] font-bold text-white flex items-center justify-center ${
          collapsed ? "absolute right-1.5 top-1.5 h-4 min-w-4 px-1" : "h-5 min-w-5 px-1.5"
        }`}>
          {badge}
        </span>
      )}
...
export const RecruiterLayout = ({ children, title, showNewJobButton = true }: { children: ReactNode; title: string; showNewJobButton?: boolean }) => {
  ...
  return (
    <div className="min-h-screen flex bg-cream">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col gap-0.5 bg-charcoal px-3 py-4 transition-all duration-300 ${sidebarCollapsed ? "w-[84px]" : "w-[228px]"}`}>
        <div className={`mb-4 flex items-center ${sidebarCollapsed ? "justify-center" : "justify-between px-2"}`}>
          <Link to="/dashboard" className={`flex items-center gap-2 ${sidebarCollapsed ? "justify-center" : ""}`}>
          <div className="h-8 w-8 rounded-lg bg-coral flex items-center justify-center relative overflow-hidden">
            ...
            {!sidebarCollapsed && <span className="font-display font-bold text-[18px] tracking-tight text-cream">HireIQ</span>}
          </Link>
          {!sidebarCollapsed && (
            <button
              type="button"
        ...
        {!sidebarCollapsed && <div className="px-3 mb-1 text-[9px] uppercase tracking-wider text-cream/50 font-semibold">Workspace</div>}
        <SidebarItem Icon={LayoutGrid} label="Dashboard" path="/dashboard" collapsed={sidebarCollapsed} />
        <SidebarItem Icon={Briefcase} label="Jobs" path="/jobs" collapsed={sidebarCollapsed} />
        <SidebarItem Icon={ClipboardList} label="Review Queue" path="/review" badge="3" collapsed={sidebarCollapsed} />
        <SidebarItem Icon={Settings} label="Settings" path="/settings" collapsed={sidebarCollapsed} />

        <div className="flex-1" />
        <div className={`border-t border-cream/10 pt-2 ${sidebarCollapsed ? "px-0" : "px-2"} flex items-center ${sidebarCollapsed ? "justify-center" : "gap-2"}`}>
      ...
      {/* Main */}
      <div className={`flex-1 flex flex-col transition-all duration-300 ${sidebarCollapsed ? "ml-[84px]" : "ml-[228px]"}`}>
        {/* Top bar */}
