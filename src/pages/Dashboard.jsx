...
const Metric = ({ label, value, tone }) => (
  <div className="rounded-2xl border border-charcoal/10 bg-white p-5">
    ...
    <div
      className={`mt-2 font-display font-bold text-3xl ${tone === "good" ? "text-coral" : tone === "bad" ? "text-destructive" : "text-charcoal"}`}
    >
      {value}
    </div>
  </div>
...
const StatusPill = ({ s }) => (
  <span
    className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-semibold ${s === "Active" ? "bg-amber/20 text-coral" : "bg-charcoal/10 text-charcoal-muted"}`}
  >
    <span
      className={`h-1.5 w-1.5 rounded-full ${s === "Active" ? "bg-amber" : "bg-charcoal-muted"}`}
    />
    {s}
  </span>
);
...
const feed = [
  ...
  {
    dot: "bg-blue-500",
    text: "Sent video invite to Jordan M. — score 74",
    time: "8 min ago",
  },
  {
    dot: "bg-amber",
  ...
  {
    dot: "bg-blue-500",
    text: "Sent video invite to Mohan R. — score 81",
    time: "3 hrs ago",
  },
];
              ...
              <table className="w-full table-fixed text-[12px]">
                <thead>
                  <tr className="text-left text-[10px] uppercase tracking-wider text-charcoal-muted">
                    ...
                    <th className="w-[9%] px-2 py-3 font-medium text-center">
                      Cand.
                    </th>
                    <th className="w-[9%] px-2 py-3 font-medium text-center">
                      Applied
                    ...
                    <th className="w-[11%] px-2 py-3 font-medium text-center">
                      AI rej.
                    </th>
                    <th className="w-[10%] px-2 py-3 font-medium text-center">
                      Short.
                ...
                <tbody>
                  {recruiterJobs.map((j) => (
                    <tr
                      key={j.id}
                      onClick={() => navigate(`/jobs/${j.id}/pipeline`)}
                      className="border-t border-charcoal/5 hover:bg-cream/40 cursor-pointer transition"
                    >
                      ...
                      <td className="px-2 py-5 text-center text-destructive font-semibold">
                        {j.aiRejected}
                      </td>
                      <td className="px-2 py-5 text-center text-charcoal font-semibold">
                        {j.shortlisted}
                      </td>
                      <td className="px-2 py-5 text-center">
                        {j.pending > 0 ? (
                          <span className="inline-flex items-center justify-center h-5 min-w-5 px-1.5 rounded-full bg-amber-soft text-amber-warm text-[10px] font-bold">
                            {j.pending}
                          </span>
                        ) : (
                          <span className="text-charcoal-muted text-[12px]">
                            0
                          </span>
                        )}
                      </td>
          ...
          <div className="lg:col-span-2 rounded-2xl bg-white border border-charcoal/10 p-5 flex flex-col">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {["All", "Rejections", "Invites", "Shortlists"].map((p, i) => (
                <button
                  key={p}
                  className={`px-3 py-1 rounded-full text-[11px] font-medium ${i === 0 ? "bg-coral text-cream" : "bg-cream text-charcoal-muted hover:text-charcoal"}`}
                >
                  {p}
                </button>
              ))}
            ...
            <ul className="mt-4 space-y-3 flex-1">
              {feed.map((f, i) => (
                <li key={i} className="flex items-start gap-2.5">
                  <span
                    className={`mt-1.5 h-2 w-2 rounded-full flex-none ${f.dot}`}
                  />
