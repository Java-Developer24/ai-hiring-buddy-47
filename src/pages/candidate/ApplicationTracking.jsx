      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <ChevronRight
                        className={`h-4 w-4 ${active ? "text-coral" : "text-charcoal-muted"}`}
                      />
                    </div>
          ...
          <div className="bg-white rounded-3xl border border-charcoal/10 shadow-sm overflow-hidden">
            ...
            <div className="p-8">
              <h3 className="text-sm font-bold text-charcoal uppercase tracking-wider mb-8">
                Journey map
              </h3>
              ...
              <div className="grid gap-5 md:grid-cols-2">
                {stages.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white bg-white shadow ${status === "completed" ? "text-coral" : status === "current" ? "text-amber" : "text-charcoal/20"}`}
                      >
                        {status === "completed" ? (
                      ...
                      <div className="mt-4 flex items-center justify-between gap-4">
                        <h4
                          className={`text-base font-bold ${status === "upcoming" ? "text-charcoal/40" : "text-charcoal"}`}
                        >
                          {s.label}
                        </h4>
                        {status === "completed" && (
                          <span className="text-[10px] font-bold uppercase tracking-wider text-coral bg-coral/10 px-2 py-0.5 rounded-md">
                            Done
                          </span>
                        )}
                        {status === "current" && (
                          <span className="text-[10px] font-bold uppercase tracking-wider text-amber bg-amber-soft px-2 py-0.5 rounded-md">
                            In Progress
                          </span>
                        )}
                      </div>
                      <time
                        className={`mt-2 block text-xs font-medium ${status === "upcoming" ? "text-charcoal/30" : "text-charcoal-muted"}`}
                      >
                        {s.date}
                      </time>
                    </div>
