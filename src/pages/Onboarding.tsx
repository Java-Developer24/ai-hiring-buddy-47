      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s.n} className="flex items-center gap-2 flex-1">
            <div className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${current ? "bg-coral text-cream" : done ? "bg-amber text-coral" : "bg-cream border border-charcoal/15 text-charcoal-muted"}`}>
              {done ? <Check className="h-3.5 w-3.5" strokeWidth={3} /> : s.n}
            </div>
            <span className={`text-xs font-medium ${current ? "text-charcoal" : "text-charcoal-muted"} hidden sm:inline`}>{s.label}</span>
            {i < steps.length - 1 && <div className={`flex-1 h-px ${done ? "bg-amber" : "bg-charcoal/15"}`} />}
          </div>
        );
      })}
    </div>
...
const Step1 = ({ onNext }: { onNext: () => void }) => (
  <Card>
    <Stepper active={1} />
    <h2 className="mt-8 font-display font-bold text-[18px] text-charcoal">Tell us about your organisation</h2>
    <p className="text-[13px] text-charcoal-muted mt-1">A few details to personalise your workspace.</p>
    ...
    <button onClick={onNext} className="mt-7 w-full h-11 rounded-xl bg-coral text-cream font-semibold text-sm hover:bg-charcoal transition flex items-center justify-center gap-2 shadow-sm">
...
const Step2 = ({ onNext }: { onNext: () => void }) => (
  <Card>
    <Stepper active={2} />
    <h2 className="mt-8 font-display font-bold text-[18px] text-charcoal">Make it yours</h2>
    <p className="text-[13px] text-charcoal-muted mt-1">Branding shown to candidates during the interview.</p>
    <div className="mt-6 grid grid-cols-2 gap-5">
      <div className="space-y-4">
        ...
        <Field label="Brand colour">
          <div className="flex items-center gap-2">
            <div className="h-9 w-9 rounded-lg border border-charcoal/15" style={{ backgroundColor: "#0D1829" }} />
      ...
      <div>
        <div className="text-xs font-medium text-charcoal mb-1.5">Live preview</div>
...
const Step3 = () => {
  ...
  return (
    <Card>
      <Stepper active={3} />
      <h2 className="mt-8 font-display font-bold text-[18px] text-charcoal">Invite your team</h2>
      <p className="text-[13px] text-charcoal-muted mt-1">They'll get an email to join your workspace.</p>
      <div className="mt-6 space-y-2.5">
        {rows.map((r, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <button onClick={() => setRows(rows.filter((_, x) => x !== i))} className="h-9 w-9 rounded-lg flex items-center justify-center text-charcoal-muted hover:bg-cream"><X className="h-4 w-4" /></button>
          </div>
        ))}
      </div>
      <button onClick={() => setRows([...rows, { email: "", role: "Recruiter" }])} className="mt-3 inline-flex items-center gap-1.5 text-xs font-semibold text-coral hover:underline">
        <Plus className="h-3.5 w-3.5" /> Add another member
      </button>
      <div className="mt-7 space-y-2">
        <button onClick={() => navigate("/dashboard")} className="w-full h-11 rounded-xl bg-coral text-cream font-semibold text-sm hover:bg-charcoal transition shadow-sm">
          Finish setup — go to dashboard
        </button>
        <button onClick={() => navigate("/dashboard")} className="w-full h-9 text-xs text-charcoal-muted hover:text-charcoal">Skip for now</button>
      </div>
    </Card>
  );
};
...
const Field = ({ label, children }: { label: string; children: React.ReactNode }) => (
  <div>
    <label className="text-xs font-medium text-charcoal mb-1.5 block">{label}</label>
    {children}
  </div>
);
