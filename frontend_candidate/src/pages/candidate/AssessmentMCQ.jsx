...
const AssessmentMCQ = () => {
  ...
  return (
    <CandidateLayout
      className="bg-cream"
      showLogo={true}
      backHref="/device-check/assessment"
          ...
          >
            {questions[currentQuestion].opts.map((opt) => (
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${selected === opt.id ? "text-coral" : "text-charcoal"}`}
                >
                  {opt.id}. {opt.text}
                </span>
              </label>
          ...
          <div className="space-y-4 pt-4">
            <button
              ...
              className={`w-full h-12 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition shadow-lg ${
                selected
                  ? "bg-[hsl(var(--charcoal))] hover:bg-charcoal shadow-[hsl(var(--charcoal))]/10"
                  : "bg-charcoal/10 cursor-not-allowed shadow-none"
              }`}
            >
              {currentQuestion === questions.length - 1
                ? "Complete the Assessment and Start with Video Interview"
                : "Next question"}{" "}
